...
task :cron_enable, :roles => :web, :except => { :no_release => true } do
  run "#{drush_path} -r #{current_docroot_dir} vset --yes elysia_cron_disabled 0", :once => true
end #cron_enable



task :cron_disable, :roles => :web, :except => { :no_release => true } do
  run "#{drush_path} -r #{current_docroot_dir} vset --yes elysia_cron_disabled 1 ", :once => true
end #cron_disable
...
task :status, :roles => :web do
  run "#{drush_path} status -r #{current_docroot_dir}", :once => true
end # status
